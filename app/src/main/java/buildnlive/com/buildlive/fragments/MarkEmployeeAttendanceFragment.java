...
import buildnlive.com.buildlive.adapters.EmployeeAttendanceAdapter;
...
public class MarkEmployeeAttendanceFragment extends Fragment {
    private Button submit,markAbsent;
    ...
    public static MarkEmployeeAttendanceFragment newInstance(App a) {
        app = a;
        return new MarkEmployeeAttendanceFragment();
    }

    @Nullable
    @Override
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_mark_attendance_emp, container, false);
    }
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        markAbsent = view.findViewById(R.id.markAbsent);
...
//                        <-------------------------- Network Request------------------------------->
                        ...
                        for (final String s : EmployeeAttendanceAdapter.ViewHolder.changedUsers.keySet()) {
...
//                            Worker u = realm.where(Worker.class).equalTo("id", s).findFirst();
                            try {
                                if (EmployeeAttendanceAdapter.ViewHolder.changedUsers.get(s)) {
                                    ...
                                    if (!PrefernceFile.Companion.getInstance(context).getString("CheckinTimeEmployee" + u.getId()).equals("")) {
                                        checkIn.put(new JSONObject()
                                                .put("starttime", PrefernceFile.Companion.getInstance(context).getString("CheckinTimeEmployee" + u.getId()))
                                                .put("labour_id", u.getWorkerId()).put("ref_no", refNo.getText().toString()));
                                    } else {
                        ...
                        if (checkIn.length() > 0) {
                            ...
                            app.sendNetworkRequest(Config.REQ_POST_EMP_CHECK_IN, Request.Method.POST, params, new Interfaces.NetworkInterfaceListener() {
                                @Override
                                ...
                                public void onNetworkRequestComplete(String response) {
                                    ...
                                    hider.setVisibility(View.GONE);
                                    utilityofActivity.dismissProgressDialog();
                                    EmployeeAttendanceAdapter.ViewHolder.changedUsers.clear();
...
//                                                PrefernceFile.Companion.getInstance(context).setString("attendence_id" + u.getId(), obj.getString("attendence_id"));
                        ...
                        if (checkOut.length() > 0) {
                            ...
                            app.sendNetworkRequest(Config.REQ_POST_EMP_CHECK_OUT, Request.Method.POST, params, new Interfaces.NetworkInterfaceListener() {
                                @Override
                                public void onNetworkRequestStart() {
                                    utilityofActivity.showProgressDialog();
                                }
...
//                                        EmployeeAttendanceAdapter.ViewHolder.check_out.setEnabled(false);
                                        for (Worker u : checkingOutWorkers) {
                                            u.setCheckOutTimeSelected(PrefernceFile.Companion.getInstance(context).getString("CheckoutTimeEmployee" + u.getId()));
...
//                        <-------------------------- Network Request------------------------------->
                        ...
                        if (absent.length() > 0) {
                            ...
                            params.put("user_id", App.userId);
                            params.put("project_id", App.projectId);
                            params.put("attendence", absent.toString()); // TODO PLEASE CHECK SPELLING MISTAKES
                            console.log("Params" + params.toString());
                            app.sendNetworkRequest(Config.MARK_LEAVE_USER, Request.Method.POST, params, new Interfaces.NetworkInterfaceListener() {
                                @Override
                                public void onNetworkRequestStart() {
                                    utilityofActivity.showProgressDialog();
                                }
                                ...
                                public void onNetworkRequestComplete(String response) {
                                    ...
                                    utilityofActivity.dismissProgressDialog();
                                    EmployeeAttendanceAdapter.ViewHolder.changedUsers.clear();
                                    console.log("Absent Response:" + response);
                                    try {
                                        JSONArray array = new JSONArray(response);
                                        Worker u = new Worker();
                                        for (int i = 0; i < array.length(); i++) {
...
//        list.setmMaxHeight(400);
    ...
    private void refresh() {
        workers.clear();
        String requestUrl = Config.REQ_GET_EMPLOYEE;
        requestUrl = requestUrl.replace("[0]", App.userId);
        ...
        app.sendNetworkRequest(requestUrl, Request.Method.GET, null, new Interfaces.NetworkInterfaceListener() {
            ...
            public void onNetworkRequestComplete(final String response) {
                ...
                try {
                    ...
                    adapter = new EmployeeAttendanceAdapter(getContext(), workers, new EmployeeAttendanceAdapter.OnItemClickListener() {
                        @Override
