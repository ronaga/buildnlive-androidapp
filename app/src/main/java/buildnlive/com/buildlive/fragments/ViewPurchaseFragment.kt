...
import buildnlive.com.buildlive.Server.Request.DeletePurchaseRequest
...
import buildnlive.com.buildlive.Server.Response.ViewPurchasingResponse
import buildnlive.com.buildlive.Server.RetrofitApiAuthSingleTon
import buildnlive.com.buildlive.Server.TCApi
import buildnlive.com.buildlive.adapters.PurchaseItemAdapter
...
class ViewPurchaseFragment : Fragment() {
    ...
    private var paymentItemAdapter: PurchaseItemAdapter? = null
    ...
    private var listener = object : PurchaseItemAdapter.OnItemInteractListener {
    ...
    fun getPurchasesList() {
        disposable.add(tcApi!!.callSendPurchasing(ProjectIdRequest(App.projectId))
                ...
                .subscribeWith(object : DisposableSingleObserver<Response<ViewPurchasingResponse>>() {
                    override fun onSuccess(t: Response<ViewPurchasingResponse>) {
                        if (t.isSuccessful) {
                            val response = t.body()
                            if (response!!.success) {
                                try {
                                    ...
                                    paymentItemAdapter = PurchaseItemAdapter(context!!, paymentList, listener)
    ...
    fun deletePayment(purchaseId: String) {
        disposable.add(tcApi!!.callDeletePurchasing(DeletePurchaseRequest(purchaseId))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .doOnSubscribe { utilityofActivity!!.showProgressDialog() }
                .doOnError { utilityofActivity!!.dismissProgressDialog() }
    ...
    companion object {
        @JvmStatic
        fun newInstance(): ViewPurchaseFragment {
            return ViewPurchaseFragment()
        }
    }
}